...
def convert_list_smile(filepath):
    list_millisec=read_list_smile(filepath)
    list_second=[]
    max=0
    for items in list_millisec:
        if(items[1]>max):
            max=items[1]
        if(items[0]%2000==0):
            list_second.append((items[0]/2000,max))
            max=0
    return list_second


def getthreadhold(filepath):
    data=convert_list_smile(filepath)
    max=0
    list=[]
    for i in data:
        list.append(i[1])
